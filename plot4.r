...
study[, 4] <- as.numeric(study[, 4])
study[, 5] <- as.numeric(study[, 5])
study[, 7] <- as.numeric(study[, 7])
study[, 8] <- as.numeric(study[, 8])
study[, 9] <- as.numeric(study[, 9])
df <- subset(study, select = c(10, 3, 4, 5, 7, 8, 9))
par(mfrow = c(2, 2))
plot(df$DT, df$Global_active_power, type = "l", xlab ="", ylab = "Global Active Power")
plot(df$DT, df$Voltage, type = "l", ylim = c(234, 246), xlab ="datetime", ylab = "Voltage")
...
plot(df$DT, df$Global_reactive_power, type = "l", ylim = c(0, 0.5), xlab ="datetime", ylab = "Global_reactive_power")
